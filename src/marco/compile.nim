import options, xmltree, xmlparser, strtabs, strutils, sets, tables, critbits
import blob, consts
...
const
  ...
  knownManifestAttrs = @[
    ...
    KnownAttr(name: "android:minSdkVersion", rawDefault: "1", typ: dtInt, stripRaw: true),
    KnownAttr(name: "android:targetSdkVersion", rawDefault: "30", typ: dtInt, stripRaw: true),
    KnownAttr(name: "android:versionCode", rawDefault: "1", typ: dtInt, stripRaw: true),
    KnownAttr(name: "android:versionName", rawDefault: "1.0", typ: dtString),
  ]
  knownResources = [
    ...
    ("name", 0x01010003'u32),
    ("minSdkVersion", 0x0101020c'u32),
    ("targetSdkVersion", 0x01010270'u32),
    ("versionCode", 0x0101021b'u32),
    ("versionName", 0x0101021c'u32),
  ].toTable
...
proc renderXML(res: var Blob, xml: XmlNode, stringsMap: CritBitTree[uint32], lineNo: var uint32) =
  ...
  let tag = xml.tag.splitQName.name
  res.put32 0xffff_ffff'u32   # TODO: handle namespaces
  res.put32 stringsMap[xml.tag.splitQName.name]
  ...
  # Render attributes
  for k, v in attrs:
    let (namespace, attr) = k.splitQName
    
    res.put32 case namespace
    of "android": stringsMap[nsAndroid]
    else: 0xffff_ffff'u32
    ...
    if tag in ["manifest", "uses-sdk"] and isKnown.isSome:
      typ = isKnown.get.typ
